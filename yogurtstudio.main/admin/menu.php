		...
		if ($GLOBALS['APPLICATION']->GetGroupRight($moduleID) >= 'R') {
			$arMenu = [
				...
				'items'    => [
					[
						...
						'sort'      => 10,
						'url'       => '/bitrix/admin/'. $module . '/' . $moduleID . '_mc.php?lang=' . urlencode(LANGUAGE_ID),
					...
					[
						...
						'sort'      => 20,
						'url'       => '/bitrix/admin/'. $module . '/' . $moduleID . '_options.php?mid=main&lang=' . urlencode(LANGUAGE_ID),
						'icon'      => 'imi_typography',
			...
			if (!\CModule::IncludeModule('aspro.smartseo')) {
				$arMenu['items'][] = [
					...
					'sort'  => 1000,
					'url'   => '/bitrix/admin/'. $module . '/' . $moduleID . '_smartseo_load.php?lang=' . urlencode(LANGUAGE_ID),
					'icon'  => 'imi_smartseo',
				];
			}
